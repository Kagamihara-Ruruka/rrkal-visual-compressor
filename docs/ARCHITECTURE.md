...
`review.json` is an optional sidecar generated by `--review-packet`. It records
source fingerprints, verification policy, package self-check, source-fidelity
metrics, and the final accepted flag. It is intended for RRKAL/editor handoff
and later audit, not for reconstructing the visual model.
