...
- `demo.py` 輸出
- `metrics.json` 輸出
- benchmark report，比較大小、誤差、壓縮率，並記錄 direct SVG gzip、source CSV、source CSV.gz、LTTB SVG raw/gzip baselines
...
Build 也可以在 package 旁邊寫出 review packet。這份 `review.json` 會記錄 source fingerprint、verification policy、package self-check、source-fidelity metrics，也會記錄 package bytes 與 source numeric array bytes，作為第一層壓縮證據。若啟用 `--direct-svg`，review 也會記錄 direct SVG raw bytes、gzip bytes，以及 baseline-to-package ratios：
...
不重新 build，也可以直接比較既有 package 與 baseline files：
...
`--defensible-channel-coverage` 只會影響「高保真可行方案」在 summary 的挑選（`best_defensible_high_fidelity_svg_gzip_candidate`），
不會改變 gate 的通過條件；若要改 gate 邏輯，仍請搭配 `--min-channel-coverage`。
